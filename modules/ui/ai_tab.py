...
def create_ai_tab(parent, app):
    ...
    ttk.Checkbutton(api_frame, text="Show API Key", variable=app.show_api_key, command=toggle_api_key_visibility).grid(row=0, column=2, padx=5, pady=5)
    
    # Test API key button
    ttk.Button(api_frame, text="Test API Connection", command=app.test_openai_connection).grid(row=1, column=1, padx=5, pady=5)
